...
const HOLISTIC_DAY_ASSIGNMENT_INSTRUCTIONS =
  ...
  "- Distance — compute it explicitly, do not eyeball it: deltaLat = lat2-lat1, deltaLng = lng2-lng1, kmPerDegLat ≈ 111, kmPerDegLng ≈ 111 × cos(midLat in radians). km ≈ sqrt((deltaLat×kmPerDegLat)² + (deltaLng×kmPerDegLng)²). Work this out per leg before picking a mode — do not guess from the raw coordinate values.\n" +
  "- Walking is capped hard at 1.5km (≈ 18-20 min on foot). NEVER choose \"walking\" above that distance, even if the venues feel close by name or the group seems active — a 267-minute walking leg is a real failure mode this rule exists to prevent. If you are not confident in your distance calculation, do not choose \"walking\" — fall back to \"driving\" or \"transit\".\n" +
  "- Roughly 1.5km–6km in a dense, transit-served city → transit (if plausible, see below) else driving. Above 6km, or in car-dependent/suburban/rural areas at any distance → driving.\n" +
  "- Transit plausibility at destination: only choose \"transit\" when the destination city/region plausibly has usable public transit for visitors (major metros and dense urban destinations). For car-dependent, suburban, rural, or small-town destinations, never choose \"transit\" — use driving instead.\n" +
  "- Budget: for budget-conscious groups (groupProfile.budgetLevel = 'budget', or members with budgetLevel='budget'/'low'), prefer transit over driving within the transit-plausible range above, since taxis/rideshare add cost. This does not relax the walking distance cap. Luxury or high-budget groups can default to driving more readily.\n" +
  "- Traffic conditions: you do not have live traffic data, but apply general judgment — dense city-center legs, and legs during typical midday/evening peak hours, tend to be slower and less predictable by car, so prefer transit when distance and destination allow. Legs between outlying areas with sparse transit are fine to drive.\n" +
  "- When signals conflict, the distance cap and transit plausibility always win over budget and traffic — never stretch \"walking\" or \"transit\" past what the distance and destination actually support.\n\n" +
