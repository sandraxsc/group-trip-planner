    ...
    <div className="flex flex-col min-h-screen bg-[#F7F7F6] pb-24">
      ...
      <div className="px-5 pt-12">
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-white rounded-2xl border-2 border-[#E8E8E8] shadow-[0_3px_0_#C4C4C4] p-1 flex">
            {(
              [
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-[#10B954] flex items-center justify-center shadow-[0_3px_0_#0D9443] active:translate-y-0.5 active:shadow-none transition-all flex-shrink-0"
          >
      ...
      <div className="px-5 pt-4 flex flex-col gap-3">
        {visible.length === 0 && (
          <div className="bg-white rounded-2xl border-2 border-dashed border-[#E8E8E8] p-8 text-center mt-2">
            <p className="text-3xl mb-2" aria-hidden>
              {tab === "ongoing" ? "🗺️" : "🏁"}
            </p>
            <p className="text-[#1F302E] font-black text-sm">
              {tab === "ongoing"
                ? "No trips in active planning"
                : "No completed trips yet"}
            </p>
            <p className="text-[#6B7280] font-bold text-xs mt-1">
              {tab === "ongoing"
            ...
            {tab === "ongoing" && (
              <button
                type="button"
                onClick={() => navigate("/create-trip")}
                className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#10B954] text-white font-black text-sm shadow-[0_3px_0_#0D9443] active:translate-y-0.5 active:shadow-none transition-all"
              >
          ...
          return (
            <button
              ...
              className={`text-left bg-white rounded-2xl border-2 ${
                isCompleted ? "border-[#E8E8E8]" : "border-[#10B954]"
              } shadow-[0_4px_0_${
                isCompleted ? "#C4C4C4" : "#0D9443"
              }] p-4 active:translate-y-0.5 active:shadow-none transition-all`}
            >
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#F7F7F6] border-2 border-[#E8E8E8] flex items-center justify-center text-2xl flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2 mb-0.5">
                    <span
                      className={`text-[10px] font-black uppercase tracking-[0.5px] px-2 py-0.5 rounded-full border ${
                        isCompleted
                          ? "text-[#0D9443] bg-[#E8F8DA] border-[#9AE16A]"
                          : "text-[#FF9F1C] bg-[#FFF8E6] border-[#FFB000]"
                      }`}
                    >
                      {isCompleted ? "Completed" : "Planning"}
                    </span>
                    {isCompleted && (
                      <CheckCircle2 size={14} className="text-[#10B954]" fill="#10B954" />
                    )}
                  </div>
                  <h3 className="font-black text-[#1F302E] text-base leading-snug break-words">
                    {trip.name}
                  </h3>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.4px]">
                    {isCompleted ? "Itinerary" : "Planning progress"}
                  </span>
                  <span
                    className={`text-[11px] font-black ${
                      isCompleted ? "text-[#0D9443]" : "text-[#10B954]"
                    }`}
                  >
